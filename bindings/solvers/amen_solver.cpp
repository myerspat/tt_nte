...
void register_AMEnSolver(py::module_& m)
{
  ...
  py::class_<AMEnSolver, LocalSolver, std::shared_ptr<AMEnSolver>>(
    ...
    // =================================================================
    // Public constructors
    .def(py::init([](int nswp, double eps, double eps_forcing, int rmax,
                    int max_full, int kickrank, int kick2, int local_iterations,
                    int resets, bool verbose, int prec) {
      return AMEnSolver::create(nswp, eps, eps_forcing, rmax, max_full,
        kickrank, kick2, local_iterations, resets, verbose, prec);
    }),
