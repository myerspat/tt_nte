...
def pu239(num_groups: Literal[1, 2], device=None, dtype=None):
    """
    ...
    if num_groups == 1:
        ...
        pu239.nu_fission = torch.tensor(
            [3.1 * 0.0936, 2.93 * 0.08544], dtype=dtype, device=device
        )
        pu239.fission = torch.tensor([0.0936, 0.08544], dtype=dtype, device=device)
        pu239.absorption = torch.tensor([0.0984, 0.09984], dtype=dtype, device=device)
...
def kaist(
    ...
    Returns
    -------
    list of ttnte.xs.MaterialLabel
        The labels for the materials in the problem.
    ttnte.xs.Server
        XS data server object.
    """
    if problem != "2B":
    ...
    data = json.load(open(Path(__file__).parent / f"data/kaist{problem}.json"))
    chi = torch.tensor(data.pop("chi"), dtype=dtype, device=device)
    ...
    for name, mat_data in data.items():
        ...
        nu_fission = torch.tensor(mat_data["nu_fission"], dtype=dtype, device=device)
        mat.chi = chi if torch.any(nu_fission != 0) else torch.zeros_like(chi)
        mat.total = torch.tensor(mat_data["total"], dtype=dtype, device=device)
        mat.absorption = torch.tensor(
            mat_data["absorption"], dtype=dtype, device=device
        )
        mat.nu_fission = nu_fission
